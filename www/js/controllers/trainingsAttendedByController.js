angular.module("TrainingsAttendedByController", ['ionic-datepicker']).
controller('TrainingsAttendedByController', ['$scope', '$http','MainService', '$ionicPopup', '$stateParams', 'ionicDatePicker', '$filter', '$timeout',
function($scope, $http, MainService, $ionicPopup, $stateParams, ionicDatePicker, $filter, $timeout){
	...
	$scope.datepickerClicked = function(f_Obj,s_Obj){
		if(!$scope.flag){
			...
			$scope.month=$scope.currentMonth;
			$scope.year=$scope.currentYear;
			var index1=$scope.trainings_attended_by_obj.activities.indexOf(f_Obj);
			var index2=$scope.trainings_attended_by_obj.activities[""+index1+""].details.indexOf(s_Obj);
			$scope.date={
				month:0,
				day:0,
				year:0
			...
			var pop = $ionicPopup.show({
			    ...
			    scope: $scope,
	     		buttons: [{
	     			text: 'Ok',
	     			onTap:function(e){
	     				$scope.trainings_attended_by_obj.activities[""+index1+""].details[""+index2+""].value = $scope.date.day+"-"+$scope.date.month+"-"+$scope.date.year;
	     				pop.close();
	     			}
	     		}]
	     	})
	...
	$scope.trainings_attended_by_obj = MainService.getForm_data();
    if($scope.trainings_attended_by_obj.submitted === undefined){
    	//fetching data from json and storing it in $scope.activities property
		// $http.get('data.json').
		$http.get('trainings_attended_by.json').	
		success(function(data){
			$scope.trainings_attended_by_obj = data.trainings_attended_by;
			
			$scope.trainings_attended_by_obj.farmerId = parseInt($stateParams.farmerId);
			$scope.trainings_attended_by_obj.farmerTimePeriodId = parseInt($stateParams.farmerTimePeriodId);
            //// getting area details from land details
            MainService.getlandDetailsData($scope.trainings_attended_by_obj.farmerId,$scope.trainings_attended_by_obj.farmerTimePeriodId);

			var activities = data.trainings_attended_by.activities;		
			var details = data.trainings_attended_by.details;
			var child_details = data.trainings_attended_by.child_details;
			var data = [];
			...
			//kichhi kara
			$scope.trainings_attended_by_obj.activities = [];
			for(var i = 0; i < activities.length;i++){
				...
				$scope.trainings_attended_by_obj.activities.push({
					name: activities[i].name,
					details: details_here
				});
			}
	     MainService.checkOrganicArea($scope.trainings_attended_by_obj.farmerId);
    ...
    }else if($scope.trainings_attended_by_obj.submitted === false){
         //// getting area details from land details
	     MainService.checkOrganicArea($scope.trainings_attended_by_obj.farmerId);
         MainService.getlandDetailsData($scope.trainings_attended_by_obj.farmerId,$scope.trainings_attended_by_obj.farmerTimePeriodId);
    	$scope.flag = false;
    ...
    $scope.validString=function(f_obj,s_obj,val){
     	MainService.validString(f_obj,s_obj,val,$scope,"trainings_attended_by_obj");
    }
    var saveFlag = false;
    $scope.save=function(){
    	MainService.save($scope.trainings_attended_by_obj, 11,$scope);
    	saveFlag = true;
    }

    $scope.$on('eventFired', function(event, data) {
        if($scope.trainings_attended_by_obj.submitted){     
			$scope.flag = true;
    ...
    $scope.submit=function(){
    	if(saveFlag){
		   	    MainService.submit($scope.trainings_attended_by_obj, 11,$scope);
		}else{
	...
	$scope.remove = function(){
        var click=true;
  		if($scope.trainings_attended_by_obj.activities.length >1){
				...
				buttons: [
			      { text: 'No' },
			      {
			        ...
			        onTap: function(e) {
			           ...
			           if (click) {
                         $scope.trainings_attended_by_obj.activities.splice($scope.trainings_attended_by_obj.activities.length-1, 1);
                         click=false;
                       }
                        confirmPopup.close();
	...
	db.get(master_data_doc_name).then(function(doc) {
		$http.get('trainings_attended_by.json').	
			success(function(data){
	            ...
	            var subjectTypeId=data.trainings_attended_by.subjectTypeId;
	...
	$scope.getChange = function(f_Obj,s_Obj,val){
		    ...
		    var index2=$scope.trainings_attended_by_obj.activities[""+index1+""].details.indexOf(s_Obj);
		      ...
		      var moafiltData = $scope.subArray.filter(function(obj) { if (val == obj.name ) {
               ...
               $scope.trainings_attended_by_obj.activities[""+index1+""].details[""+index2+""].value=val;
		       $scope.trainings_attended_by_obj.activities[""+index1+""].details[""+index2+""].id=moafiltData[0].typeDetailId;
	};
	$scope.changeNoc = function(status_index){
      angular.forEach($scope.subArray,function(f_value,f_key){
      	if (status_index==f_key) {
      	$scope.subArray[f_key].checked = true;
        }else{$scope.subArray[f_key].checked = false;}
      })
	};
	$scope.myfy=function(f_Obj,s_Obj,val){
		...
		var index1=$scope.trainings_attended_by_obj.activities.indexOf(f_Obj);
		var index2=$scope.trainings_attended_by_obj.activities[""+index1+""].details.indexOf(s_Obj);
		$scope.trainings_attended_by_obj.activities[""+index1+""].details[""+index2+""].value=$filter('date')(val, 'yyyy-MM-dd'); //$filter('date')(val, 'yyyy-MM-dd'); 

	}
  	$scope.add = function(){
  		$http.get('trainings_attended_by.json').	
		success(function(data){
			var trainings_attended_by_obj2 = data.trainings_attended_by;
			
			trainings_attended_by_obj2.farmer_id = parseInt($stateParams.farmer_id);
			trainings_attended_by_obj2.financial_year = parseInt($stateParams.financial_year);
			...
			trainings_attended_by_obj2.activities = [];
			for(var i = 0; i < activities.length;i++){
				activities[i].name += ($scope.trainings_attended_by_obj.activities.length + 1);
				var details_here = [];
				for(var j = 0;j <data.length;j++){
					details_here.push({
						key: data[j].key,
				...
				trainings_attended_by_obj2.activities.push({
					name: activities[i].name,
					details: details_here
				});
			}

			$scope.trainings_attended_by_obj.activities.push(trainings_attended_by_obj2.activities[0]);	
			$timeout(function(){
				$scope.shownGroup = trainings_attended_by_obj2.activities[0];
			},200);
	}).
	error(function(err){
		//error fetching json
	...
	*/
	MainService.setFlagForSyncedFarmer($scope, trainingFarmerProcessId);
}]);